...
use App\laranvifood_i;

use Validator;
class laranvifood_i_AddController extends Controller
{
	...
	public function execute(Request $request){
				...
				if($request->isMethod('post')){
					...
					if($validator->fails()){
						return redirect('admin/laranvifood_i_i')->with('status','Ошибка :C		
					...
					$vhod = new laranvifood_i();
					$vhod->fill($input);
					//dd($input);

					if($vhod->save()){
					return redirect('admin/laranvifood_i_i')->with('status','Запись Добавлена');
					}
				}
