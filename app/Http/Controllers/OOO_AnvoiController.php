...
class naming{
    ...
    static $title='ООО Анвой';
} 
class OOO_AnvoiController extends Controller
{
        //Получение Входящих
    public function getInbox(){
       if(view()->exists('admin.vhods_standart')){
           $vhods=\App\v_ooo_anvoi::all();
   ...
   public function addInbox(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
                   return redirect('admin/anvoy')->with('Error','Ошибка :C		
           ...
           $vhod = new v_ooo_anvoi();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/anvoy')->with('status','Запись Добавлена');
           }
       }
...
abort(404);
   ...
   public function addOutgoing(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
               return redirect('admin/anvoy_i')->with('Error','Ошибка :C		
                   Скорее всего не заполнили все поля');
           }
           if($request->hasFile('urn_document')){
           ...
           $vhod = new i_ooo_anvoi();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/anvoy_i')->with('status','Запись Добавлена');
           }
       }
