...
use App\i_ooo_domtepla;
use App\v_ooo_domtepla;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО Дом Тепла';
}  

class OOO_DomTeplaController extends Controller
{
    //Получение Входящих
    public function getInbox(){
       if(view()->exists('admin.vhods_standart')){
           $vhods=\App\v_ooo_domtepla::all();
   ...
   public function addInbox(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
                   return redirect('admin/domtepla')->with('Error','Ошибка :C		
           ...
           $vhod = new v_ooo_domtepla();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/domtepla')->with('status','Запись Добавлена');
           }
       }
...
abort(404);
   ...
   public function addOutgoing(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
               return redirect('admin/domtepla_i')->with('Error','Ошибка :C		
                   Скорее всего не заполнили все поля');
           }
           if($request->hasFile('urn_document')){
           ...
           $vhod = new i_ooo_domtepla();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/domtepla_i')->with('status','Запись Добавлена');
           }
       }
