...
use App\i_ip_voitikva;
use App\v_ip_voitikva;
use Validator;
class naming{
    //Название контроллера
    static $title='ИП Войтик В.А.';
}  
class IP_VoitikVAController extends Controller
{
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_ip_voitikva::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/voitikva')->with('Error','Ошибка :C		
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/voitikva_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_ip_voitikva();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/voitikva_i')->with('status','Запись Добавлена');
            }
        }
