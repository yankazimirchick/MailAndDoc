...
use App\i_oao_laranvi;
use App\v_oao_laranvi;
use Validator;
class naming{
    //Название контроллера
    static $title='ОАО Ларанви';
}  
class OAO_LaranviController extends Controller
{ 
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_oao_laranvi::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/laranvi')->with('Error','Ошибка :C		
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/laranvi_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
