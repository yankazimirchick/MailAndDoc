...
use App\i_up_softgroup;
use App\v_up_softgroup;
use Validator;
class naming{
    //Название контроллера
    static $title='УП СОФТГРУПП';
}  
class UP_SoftGroupController extends Controller
{
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_up_softgroup::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/softgroup')->with('Error','Ошибка :C		
            ...
            $vhod = new v_up_softgroup();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/softgroup')->with('status','Запись Добавлена');
            }
        }
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/softgroup_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_up_softgroup();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/softgroup_i')->with('status','Запись Добавлена');
            }
        }
