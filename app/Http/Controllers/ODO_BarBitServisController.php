...
use App\i_ooo_barbitservis;
use App\v_ooo_barbitservis;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО БарановичиБытСервис';
}  
class OOO_BarBitServisController extends Controller
{  
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_ooo_barbitservis::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/barbitser')->with('Error','Ошибка :C		
            ...
            $vhod = new v_ooo_barbitservis();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/barbitser')->with('status','Запись Добавлена');
            }
        }
 ...
 abort(404);
    ...
    public function getOutgoing(){
        ...
        if(view()->exists('admin.ishods_standart')){
            $vhods=\App\i_ooo_barbitservis::all();
            $vhods = $vhods->sortByDesc('created_at');
            $data=[
             'title' => $title,
             'spisok' => $vhods
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/barbitser_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_ooo_barbitservis();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/barbitser_i')->with('status','Запись Добавлена');
            }
        }
