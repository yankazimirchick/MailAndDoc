...
use App\i_ooo_mariinski;
use App\v_ooo_mariinski;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО Отель Мариинский';
}  
class OOO_MariinskiController extends Controller
{
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_ooo_mariinski::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/mariinsky')->with('Error','Ошибка :C		
            ...
            $vhod = new v_ooo_mariinski();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/mariinsky')->with('status','Запись Добавлена');
            }
        }
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/mariinsky_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
