...
use App\i_oao_laranvifood;
use App\v_oao_laranvifood;
use Validator;
class naming{
    //Название контроллера
    static $title='ОАО ЛаранвиФуд';
}  
class OAO_LaranviFoodController extends Controller
{
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_oao_laranvifood::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/laranvifood')->with('Error','Ошибка :C		
 ...
 abort(404);
    ...
    public function getOutgoing(){
        ...
        if(view()->exists('admin.ishods_standart')){
            $vhods=\App\i_oao_laranvifood::all();
            $vhods = $vhods->sortByDesc('created_at');
            $data=[
             'title' => $title,
             'spisok' => $vhods
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/laranvifood_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_oao_laranvifood();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/laranvifood_i')->with('status','Запись Добавлена');
            }
        }
