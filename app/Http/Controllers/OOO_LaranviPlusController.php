...
use App\i_ooo_laranviplus;
use App\v_ooo_laranviplus;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО ЛаранвиПлюс';
}  
class OOO_LaranviPlusController extends Controller
{
    //Получение Входящих
    public function getInbox(){
       if(view()->exists('admin.vhods_standart')){
           $vhods=\App\v_ooo_laranviplus::all();
   ...
   public function addInbox(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
                   return redirect('admin/laranviplus')->with('Error','Ошибка :C		
           ...
           $vhod = new v_ooo_laranviplus();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/laranviplus')->with('status','Запись Добавлена');
           }
       }
...
abort(404);
   ...
   public function getOutgoing(){
       ...
       if(view()->exists('admin.ishods_standart')){
           $vhods=\App\i_ooo_laranviplus::all();
           $vhods = $vhods->sortByDesc('created_at');
           $data=[
            'title' => $title,
            'spisok' => $vhods
   ...
   public function addOutgoing(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
               return redirect('admin/laranviplus_i')->with('Error','Ошибка :C		
                   Скорее всего не заполнили все поля');
           }
           if($request->hasFile('urn_document')){
