...
use App\i_oao_gosthozvo;
use App\v_oao_gosthozvo;
use Validator;
class naming{
    //Название контроллера
    static $title='ОАО Гостиничное Хозяйство';
}  
class OAO_GostHozVoController extends Controller
{
   
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_oao_gosthozvo::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/gosthozvo')->with('Error','Ошибка :C		
            ...
            $vhod = new v_oao_gosthozvo();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/gosthozvo')->with('status','Запись Добавлена');
            }
        }
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/gosthozvo_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_oao_gosthozvo();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/gosthozvo_i')->with('status','Запись Добавлена');
            }
        }
