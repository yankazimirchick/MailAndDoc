...
use App\ritailaktive_i;
use Validator;

class ritail_vhods_i_AddController extends Controller
{
	...
	public function execute(Request $request){
				...
				if($request->isMethod('post')){
					...
					if($validator->fails()){
						return redirect('admin/ritail_vhods_i')->with('status','Ошибка :C		
					...
					$vhod = new ritailaktive_i();
					$vhod->fill($input);
					//dd($input);

					if($vhod->save()){
					return redirect('admin/ritail_vhods_i')->with('status','Запись Добавлена');
					}
				}
