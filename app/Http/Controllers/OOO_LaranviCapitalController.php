...
use App\i_ooo_laranvicapital;
use App\v_ooo_laranvicapital;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО Ларанви Капитал';
}  
class OOO_LaranviCapitalController extends Controller
{
    //Получение Входящих
    public function getInbox(){
       if(view()->exists('admin.vhods_standart')){
           $vhods=\App\v_ooo_laranvicapital::all();
   ...
   public function addInbox(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
                   return redirect('admin/laranvikapitals')->with('Error','Ошибка :C		
...
abort(404);
   ...
   public function getOutgoing(){
       ...
       if(view()->exists('admin.ishods_standart')){
           $vhods=\App\i_ooo_laranvicapital::all();
           $vhods = $vhods->sortByDesc('created_at');
           $data=[
            'title' => $title,
            'spisok' => $vhods
   ...
   public function addOutgoing(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
               return redirect('admin/laranvikapitals_i')->with('Error','Ошибка :C		
                   Скорее всего не заполнили все поля');
           }
           if($request->hasFile('urn_document')){
           ...
           $vhod = new i_ooo_laranvicapital();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/laranvikapitals_i')->with('status','Запись Добавлена');
           }
       }
