...
use App\i_ooo_newfood;
use App\v_ooo_newfood;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО НьюФуд';
}  
class OOO_NewFoodController extends Controller
{
    //Получение Входящих
    public function getInbox(){
        if(view()->exists('admin.vhods_standart')){
            $vhods=\App\v_ooo_newfood::all();
    ...
    public function addInbox(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                    return redirect('admin/newfood')->with('Error','Ошибка :C		
 ...
 abort(404);
    ...
    public function addOutgoing(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($validator->fails()){
                return redirect('admin/newfood_i')->with('Error','Ошибка :C		
                    Скорее всего не заполнили все поля');
            }
            if($request->hasFile('urn_document')){
            ...
            $vhod = new i_ooo_newfood();
            $vhod->fill($input);
            //dd($input);
 
            if($vhod->save()){
            return redirect('admin/newfood_i')->with('status','Запись Добавлена');
            }
        }
