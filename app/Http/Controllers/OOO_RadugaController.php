...
use App\i_ooo_raduga;
use App\v_ooo_raduga;
use Validator;
class naming{
    //Название контроллера
    static $title='ООО Радуга';
}  
class OOO_RadugaController extends Controller
{
    //Получение Входящих
    public function getInbox(){
       if(view()->exists('admin.vhods_standart')){
           $vhods=\App\v_ooo_raduga::all();
   ...
   public function addInbox(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
                   return redirect('admin/raduga')->with('Error','Ошибка :C		
           ...
           $vhod = new v_ooo_raduga();
           $vhod->fill($input);
           //dd($input);

           if($vhod->save()){
           return redirect('admin/raduga')->with('status','Запись Добавлена');
           }
       }
...
abort(404);
   ...
   public function addOutgoing(Request $request){
       ...
       if($request->isMethod('post')){
           ...
           if($validator->fails()){
               return redirect('admin/raduga_i')->with('Error','Ошибка :C		
                   Скорее всего не заполнили все поля');
           }
           if($request->hasFile('urn_document')){
