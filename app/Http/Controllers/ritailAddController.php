...
class ritailAddController extends Controller
{
    public function execute(Request $request){
				...
				if($request->isMethod('post')){
					...
					if($validator->fails()){
							return redirect('admin/ritail')->with('status','Ошибка :C		
					...
					if($vhod->save()){
					return redirect('admin/ritail')->with('status','Запись Добавлена');
					}
				}
